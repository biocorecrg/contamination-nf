...
// This should be adapted to single
if (params.protocol == "paired"){
  Channel
      .fromFilePairs( params.reads )
      .ifEmpty { error "Cannot find any reads matching: ${params.reads}" }
      .into { read_files_for_kraken2_1; read_files_for_kraken2_2 }
} else {
    Channel
...
process kraken2 {
  ...
  input:
  set pair_id, file(reads) from (read_files_for_kraken2_1)

  output:
  file("kraken2*.report") into kraken2_report
  file("kraken2*.out") into kraken2_output
  file("cfs*.fq.gz") into kraken2_classified
  file("ucfs*.fq.gz") into kraken2_unclassified

  script:

  if(params.protocol == "single")
      """
    	kraken2 --db ${params.kraken2db} --report kraken2_${pair_id}.report --threads ${task.cpus} ${reads} --classified-out cfs_${pair_id}.fq --unclassified-out ucfs_${pair_id}.fq > kraken2_${pair_id}.out
    	gzip *.fq
      """
  else
      """
    	kraken2 --db ${params.kraken2db} --report kraken2_${pair_id}.report --threads ${task.cpus} --paired ${reads} --classified-out cfs_${pair_id}#.fq --unclassified-out ucfs_${pair_id}#.fq > kraken2_${pair_id}.out
    	gzip *.fq
      """
}
